...
var headerApp = new Vue({
    ...
    mounted: function () {
        ...
        http.get('/public/getWdInfo', null, function (res) {
            that.logoImg = imgServer + res.data.info_logo
            that.bannerList = res.data.info_banner
            window.info_banner_time = res.data.info_banner_time * 1000
            new Vue({
                el: '#fcopyright',
                data: {
                    footerText: res.data.info_footer
                }
            })
        })
        if (!!getToken()) {
        ...
        // 判断商品列表
        if (location.href.indexOf('shop_list.html') != -1) {
            if (!GetRequest().type) {
                this.subnavText = "商品列表"
            } else {
                this.subnavText = GetRequest().type
            }
        }
    },
    methods: {
        getCart: function () {
            ...
            http.get('/getCart', null, function (res) {
                that.cartCount = res.data.qty
            })
        },
        logout: function () {
            delToken();
            localStorage.removeItem('userName')
            window.location.href = "http://localhost/wdshop/"
        },
        searchPro: function () {
            var that = this;
            window.location.href = "http://localhost/wdshop/shop_list.html?type=商品列表&keyword=" + that.keyWord
        }
    }
})